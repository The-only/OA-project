...
<div style="width: 100%">
	...
	<div style="float: right; width: 85%; margin-top: 20px;">
		<div clss="top">
			...
			<div class="container">
				<!-- 查询 -->
					<form action="getOthersInfo_leader.action?vo.cur=1" class="form-inline well" role="form"
					method="post">
				...
				<table class="table table-striped table-bordered">
					...
					<c:forEach items="${vo.users}" var="users">
						<tr>
							...
							<td>${users.address }</td>
							<td><a
								href="personalInfoOthersDetail_staff.action?vo.userid=${users.userid }&vo.cur=${vo.cur}">详情</a></td>
						</tr>
					</c:forEach>
				</table>
				<!-- 分页 -->
				<ul class="pagination">
					...
					<li><a href="personalInfoOthersAll_staff.action?vo.cur=1">首页</a></li>
					<c:if test="${vo.cur==1}">
						<li class="disabled"><a>上一页</a></li>
					</c:if>
					<c:if test="${vo.cur!=1}">
						<li><a href="personalInfoOthersAll_staff.action?vo.cur=${vo.cur-1}">上一页</a></li>
					</c:if>
					<c:if test="${vo.cur==vo.total}">
						<li class="disabled"><a>下一页</a></li>
					</c:if>
					<c:if test="${vo.cur!=vo.total}">
						<li><a href="personalInfoOthersAll_staff.action?vo.cur=${vo.cur+1}">下一页</a></li>
					</c:if>
					<li><a href="personalInfoOthersAll_staff.action?vo.cur=${vo.total}">尾页</a></li>



					<li><a>
							<form class="form-inline" name="fenyeStaff"
								action="personalInfoOthersAll_staff?vo.cur=${curr }" method="post">
...
<script>
	function change_select() {
		...
		a.action = "personalInfoOthersAll_staff.action?vo.cur=1";
	}
