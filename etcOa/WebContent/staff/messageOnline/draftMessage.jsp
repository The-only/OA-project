	...
	<div style="float: right; width: 85%;">
		<div class="container top">
			...
			<form class="form-inline well" role="form"
				action="draftMessagesByReceive_staff.action?vo.cur=1" method="post">
			...
			<table class="table table-striped">
				...
				<c:forEach items="${vo.draftMessages}" var="draftMessage">
					<tr>
						<td><a
							href="editMessage_staff.action?vo.umessid=${draftMessage.umessid }">${draftMessage.message.title }</a></td>
						<td>${draftMessage.usersByReceiveid.name }</td>
						<td>${draftMessage.message.time }</td>
						<td><a
							href="deleteMessage_staff.action?vo.umessid=${draftMessage.umessid}"
			...
			<ul class="pagination">
				...
				<li><a href="draftMessage_staff.action?vo.cur=1">首页</a></li>
				<c:if test="${vo.cur==1}">
					<li class="disabled"><a>上一页</a></li>
				</c:if>
				<c:if test="${vo.cur!=1}">
					<li><a href="draftMessage_staff.action?vo.cur=${vo.cur-1}">上一页</a></li>
				</c:if>
				<c:if test="${vo.cur==vo.total}">
					<li class="disabled"><a>下一页</a></li>
				</c:if>
				<c:if test="${vo.cur!=vo.total}">
					<li><a href="draftMessage_staff.action?vo.cur=${vo.cur+1}">下一页</a></li>
				</c:if>
				<li><a href="draftMessage_staff.action?vo.cur=${vo.total}">尾页</a></li>



				<li><a>
						<form class="form-inline" name="draftMessageStaff"
							action="draftMessage_staff.action" method="post">
